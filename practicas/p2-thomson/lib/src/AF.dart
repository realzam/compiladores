...
enum _Estado_tipo { inicial, fin, normal }

class _Transicion {
  int _inicio;
  int _fin;
  String _simbolo;
  _Transicion(int inicio, int fin, String simbolo) {
    this._inicio = inicio;
    this._fin = fin;
...
class _Estado {
  int etiqueta;
  _Estado_tipo estado_tipo;
  _Estado(this.etiqueta, {this.estado_tipo = _Estado_tipo.normal});
  List<_Transicion> tranciones = List();
  void agregar_Transicion(int estado, String simbolo) {
    _Transicion tran = _Transicion(etiqueta, estado, simbolo);
    tranciones.add(tran);
  }
...
class _AF {
  ...
  void cargar_desde(String nombre) {
    ...
    try {
      ...
      _edoInicial = inicio;
      _Estado inicial = _Estado(inicio, estado_tipo: _Estado_tipo.inicial);
      ...
      tempFinales.forEach((element) {
        ...
        _Estado finalE = _Estado(auxfin, estado_tipo: _Estado_tipo.fin);
        _estados.add(finalE);
      });
    } catch (e) {
      throw Exception('Formato del archivo no valido');
    ...
    for (var i = 2; i < contenido.length; i++) {
      ...
      if (index == -1) {
        _Estado estadoAdd = _Estado(estadoAux);
  ...
  void agregar_transicion(int inicio, int fin, String simbolo) {
    ...
    if (index == -1) {
      _Estado estadoAdd = _Estado(fin);
      _estados.add(estadoAdd);
    }
    index = _buscarEstado(inicio);
    if (index == -1) {
      _Estado estadoAdd = _Estado(inicio);
...
// Metodos auxiliares
  ...
  bool _avanzar(_Estado estado) {
    if (estado.tranciones.isEmpty && estado.estado_tipo != _Estado_tipo.fin) {
      return false;
    }
    for (_Transicion trans in estado.tranciones) {
      if (trans._fin != estado.etiqueta) return true;
    }
    return false;
  }

  _Transicion _radomTransicion(_Estado estado) {
    var rng = Random();
    int random = rng.nextInt(estado.tranciones.length);
    return estado.tranciones[random];
  }

  String _generar_letra(String cadena, _Estado estado) {
    if (!_avanzar(estado)) return null;
    _Transicion trancision = _radomTransicion(estado);
  ...
  bool _acepta(String cadena, _Estado estado) {
    if (!_avanzar(estado)) return false;
    for (_Transicion transicion in estado.tranciones) {
      if (transicion.simbolo == cadena[0] || transicion.simbolo == 'E') {
        int index = _buscarEstado(transicion.fin);
        if (index == -1) return false;
        _Estado edoAux = _estados[index];
        String cadenaAux = '';
        if (edoAux.estado_tipo == _Estado_tipo.fin && cadena.length == 1)
          return true;
